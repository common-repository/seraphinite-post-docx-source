...
class Ui
{
	...
	static function InputBox( $type, $id, $value = null, $attrs = null, $addNames = false )
	{
		...
		if( $masked )
		{
			...
			$attrs[ 'onchange' ] = 'jQuery(this.parentNode).find("input[type=\\"hidden\\"]").val(seraph_pds.Ui._MaskEncode(this.value))';
	...
	static function ItemsList_OperateBtnsTpl( array $prms, $attrs = null )
	{
		...
		$res .= Ui::Button( esc_html_x( 'AddItemBtn', 'admin.Common_ItemsList', 'seraphinite-post-docx-source' ), false, null, null, 'button', array_merge( $attrs, Gen::GetArrField( $prms, 'btnsOperate.add.attrs', array() ) ) );
		$res .= Ui::Button( esc_html_x( 'DelAllItemsBtn', 'admin.Common_ItemsList', 'seraphinite-post-docx-source' ), false, null, null, 'button', array_merge( $attrs, Gen::GetArrField( $prms, 'btnsOperate.delAll.attrs', array() ) ) );
	...
	{
		return( Ui::Tag( 'span', esc_html_x( 'NoItemsInfo', 'admin.Common_ItemsList', 'seraphinite-post-docx-source' ), $attrs ) );
	}
	...
	static function PostBoxes( $title, $metaBoxes = array( 'body' => null ), array $callbacks = null, $callbacks_args = null, $blocksAttrs = null )
	{
		...
		echo( Ui::ViewInitContent( '.wrap.seraph_pds' ) );
	}

	static function ViewInitContent( $viewCssSelector )
	{
		return( Ui::ScriptInlineContent( 'document.addEventListener("DOMContentLoaded",function(){seraph_pds.Ui.Init(jQuery("' . $viewCssSelector . '").get(0));})' ) );
	}
	...
	static function PostBoxes_Popup( $id, $title, $callback = null, $callbacks_args = null )
	{
		...
		<script>
			(function()
			{
				...
				closeBtn.on( "click", function(){ seraph_pds.Ui.PopupClose( "<?php echo( esc_attr( $id ) ); ?>" ); } );
			})();
		</script>
	...
	static function BannerMsg( $severity, $text, $opts = 0, $attrs = NULL )
	{
		...
		if( !( $opts & Ui::MsgOptDismissible ) )
			$class .= ' seraph_pds_dismiss_hidden';
